...
def tracking_configuration_detail(request, pk):
    ...
    context = {"tracking_configuration": tracking_configuration}
...
def tracking_configuration_delete(request):
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "TrackingConfiguration does not exist"}
        )
    except Exception as e:
...
def candle_list_view(request, tracking_configuration_id):
    ...
    return render(
        request, "tracking/candle_list.html", {"configuration": configuration}
    )
    ...
    try:
        if request.method == "GET":
            ...
            queryset = Candle.objects.filter(
                tracking_configuration_id=tracking_configuration_id
            ).order_by("-time")
